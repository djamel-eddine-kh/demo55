...
import come.example.dto.LigneCloture;
import come.example.dto.TotalClotureTourneeDto;
...
public class ClotureTourneeReport extends AbstractReport {
    ...
    public ClotureTourneeReport(List<LigneCloture> lignes, TotalClotureTourneeDto totaux, CompanyInfoDto companyInfoDto, List<Parameters> configs) {
    ...
    public JasperReportBuilder build() {
        return report()
                ...
                .title(createCompanyHeaderComponent(companyInfo))
    ...
    protected ComponentBuilder<?, ?> createPageHeaderComponent() {
        ...
        return cmp.verticalList(
                cmp.horizontalList(cmp.text(getResourceLabel("date") + " " + currentDate).setStyle(stl.style().bold())),
                cmp.verticalGap(10)
        );
    }
   ...
   protected ComponentBuilder<?, ?> createCompanyHeaderComponent(CompanyInfoDto companyInfo) {
       ...
       return cmp.horizontalList(
                       ...
                       cmp.text(getResourceLabel(reportTitle))
                               .setHorizontalTextAlignment(HorizontalTextAlignment.CENTER)
                               .setStyle(stl.style().bold().setFontSize(titleFontSize).setVerticalTextAlignment(VerticalTextAlignment.MIDDLE))
               )
               .newRow()
               .add(cmp.line())
               .newRow()

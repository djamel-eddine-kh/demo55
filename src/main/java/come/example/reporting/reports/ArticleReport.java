...
import come.example.dto.ArticleDto;
...
public class ArticleReport extends AbstractReport {
    private final List<come.example.dto.ArticleDto> articleData;
    ...
    public ArticleReport(List<ArticleDto> articleData, List<Parameters> parameters, CompanyInfoDto companyInfo) {
    ...
    public JasperReportBuilder build() {
        return report()
                ...
                .title(createCompanyHeaderComponent(companyInfo),createTitleComponent(),createPageHeaderComponent())
        ...
        return cmp.verticalList(
                cmp.text(getResourceLabel("depot") + " " + articleData.get(0).getNomDepot() )
                        .setStyle(stl.style().bold().setFontSize(titleFontSize)
                                .setHorizontalTextAlignment(HorizontalTextAlignment.CENTER).setTopPadding(6)),
                cmp.verticalGap(10)
        );
    }
